    ...
    async def vectorize_query(
        ...
        try:
            if embedder in self.embedders:
                ...
                if isinstance(embedder_section, dict):
                    components = embedder_section.get("components", {})
                    component = components.get(embedder, {})
                    config = component.get("config", {}) if isinstance(component, dict) else component.config
                else:
                    config = embedder_section.components[embedder].config
...
class RetrieverManager:
    ...
    async def retrieve(
        ...
        try:
            ...
            embedder_section = rag_config["Embedder"]
            if isinstance(embedder_section, dict):
                embedder_selected = embedder_section.get("selected", "")
                components = embedder_section.get("components", {})
                component = components.get(embedder_selected, {})
                component_config = component.get("config", {}) if isinstance(component, dict) else component.config
                model_field = component_config.get("Model", {}) if isinstance(component_config, dict) else component_config.get("Model", {})
                embedder_model = model_field.get("value", "") if isinstance(model_field, dict) else getattr(model_field, 'value', "")
            else:
                embedder_model = embedder_section.components[embedder_section.selected].config["Model"].value
            
            # Same for retriever config
            retriever_section = rag_config["Retriever"]
            if isinstance(retriever_section, dict):
                ret_components = retriever_section.get("components", {})
                ret_component = ret_components.get(retriever, {})
                config = ret_component.get("config", {}) if isinstance(ret_component, dict) else ret_component.config
            else:
                config = retriever_section.components[retriever].config
...
class GeneratorManager:
    ...
    async def generate_stream(self, rag_config, query, context, conversation):
        ...
        if isinstance(generator_section, dict):
            generator = generator_section.get("selected", "")
            components = generator_section.get("components", {})
            component = components.get(generator, {})
            generator_config = component.get("config", {}) if isinstance(component, dict) else component.config
        else:
            generator = generator_section.selected
            generator_config = generator_section.components[generator].config
