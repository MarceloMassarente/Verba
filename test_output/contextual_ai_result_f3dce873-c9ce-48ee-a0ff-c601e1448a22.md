...
The image contains the logo of the company "weaviate". The logo consists of a stylized "W" symbol in yellow and green, followed by the company name in white text. The background is a dark blue color.
[FIGURE_CAPTION END]
...
[FIGURE_CAPTION START]
**High level description**
The diagram illustrates a RAG pipeline consisting of stages such as Indexing, Pre-Retrieval, Retrieval, and Post-Retrieval. Each stage is associated with a set of optimization techniques. The text also lists specific optimization techniques for each stage, along with page numbers indicating where to find more information about each technique. For example, Indexing Optimization Techniques are discussed on page 3, while Post-retrieval Optimization Techniques are discussed on page 10.
...
[FIGURE_CAPTION START]
**High level description**
The flowchart depicts the RAG pipeline, starting with "Documents" which are then processed into "Chunks". A "Query" is used to retrieve relevant information. The "Embedding Model" transforms the query and chunks into vector embeddings, which are stored in the "Vector Database". The "Context" from the database, along with a "Prompt Template", is fed into the "LLM" (Large Language Model) to generate a "Response". The arrows indicate the flow of information between these components.
[FIGURE_CAPTION END]
...
[FIGURE_CAPTION START]
**High level description**
The diagram illustrates LLM-based chunking. The process starts with "Input text" which contains the sentences "Alex visited the library. He loves reading.". This input is then processed by an LLM, resulting in "Propositions" which are the same two sentences: "Alex visited the library." and "Alex loves reading."
...
[FIGURE_CAPTION START]
**High level description**
The diagram shows a set of concentric dashed circles. There are several points, some colored in light blue and some in white. The light blue points are clustered near the center of the circles, while the white points are located further away, some lying on or near the outermost circle. This diagram likely illustrates a concept related to distance or grouping, as suggested by the surrounding text.
[FIGURE_CAPTION END]
...
[FIGURE_CAPTION START]
**High level description**
This scatter plot visualizes customer reviews. The x and y axes are not explicitly labeled, but the position of each review suggests a relative rating of quality and satisfaction. Green data points represent positive reviews such as "Quality is amazing! Will definitely buy again!", "Love the design, but it broke in one week.", and "The movie was fun, but I wouldn't see it again.". Purple data points represent negative reviews such as "Food was decent, but service was terrible" and "Horrible quality. Lasted two days."
[FIGURE_CAPTION END]
...
**High level description**
The flowchart illustrates the re-ranking process, starting with a "Query" that goes through an "Embedding Model" and "Vector Database". The "Retrieved Context" is then re-ranked using a "Reranker Model" to produce a "Re-ranked Context". This context is fed into a "Prompt Template" and then an "LLM" to generate a "Response". The re-ranking stage is highlighted in green.
[FIGURE_CAPTION END]
...
[FIGURE_CAPTION START]
**High level description**
The diagram illustrates a process where a "Sentence embedding" is connected to a "Window (n sentences before and after sentence)", which is then connected to an "LLM". This process is shown twice, once on a light blue background and once on a light green background. The diagram appears to represent a workflow or architecture for processing sentences using a language model, where the window represents the context around a sentence.
[FIGURE_CAPTION END]
...
[FIGURE_CAPTION START]
**High level description**
The figure is a flowchart that depicts the Chain of Thought (CoT) prompting technique. The process starts with a "Prompt", which leads to a "Thought". This step can be repeated multiple times as indicated by the ellipsis. Finally, the process concludes with a "Response". The arrows indicate the flow of information from one step to the next.
[FIGURE_CAPTION END]
